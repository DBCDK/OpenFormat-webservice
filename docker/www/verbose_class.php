...
class verbose {
  ...
  /**
   * \brief Parse string into json
   * @param string $str
   * @param string $vtext
   * @return string
   * */
  private function json_parse($str, $vtext) {
    $str = trim($str);
    $arr['severity'] = $vtext;
    $arr['pid'] = self::$my_pid;
    $arr['date'] = date(self::$date_format);
    $arr['tracking_id'] = self::$tracking_id;
    $arr['string'] = $str;
    $pos = strpos($str, 'Total:');
    if ($pos !== FALSE) {
      list($beg, $end) = preg_split('/(?<=.{'.$pos.'})/', $str, 2);
      $timers = explode(' ', $end);
      $total = explode(':', $timers[0]);
      $js_server = explode(':', $timers[1]);
      $arr['total'] = $total[1];
      $arr['js_server'] = $js_server[1];
      $arr['string'] = $beg;
    }
